...
Quality assurance infrastructure for the **Codex Game Studio** framework.
Tests the skills and agents themselves — not any game built with them.

> **This folder is self-contained and optional.**
> Game developers using Codex Game Studio don't need it. To remove it entirely:
> `rm -rf "Codex Skill Testing Framework"` — nothing in `.codex/` depends on it.
...
├── catalog.yaml           ← master registry for covered skills and agents, coverage tracking
...
/skill-test static all              # Check all covered workflow skills
```
...
```bash
rm -rf "Codex Skill Testing Framework"
```
