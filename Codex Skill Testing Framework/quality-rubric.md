...
| **T2 — Parallel where independent** | Agents whose inputs don't depend on each other are spawned in parallel (single message, multiple Codex subagent calls) |
...
| **D4 — Deep model tier** | Agent is assigned Deep model per coordination-rules.md |
...
| **L3 — Standard model tier** | Agent is assigned Standard model (default) per coordination-rules.md |
