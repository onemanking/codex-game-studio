...
against sprint stories, and uses `Codex user-input prompt` to batch-verify manual smoke
...
5. Uses `Codex user-input prompt` for Batch 1 (core stability) and Batch 2 (sprint mechanics)
...
- [ ] `Codex user-input prompt` is used for manual smoke check batches
...
### Case 3: Manual Confirmation — Codex user-input prompt used, PASS WITH WARNINGS
...
3. `Codex user-input prompt` is used for Batch 1 and Batch 2 — developer confirms all PASS
...
**Assertions:**
- [ ] `Codex user-input prompt` is used for manual smoke check batches (not inline text prompts)
...
- [ ] Uses `Codex user-input prompt` for all manual smoke check batches (Batch 1, Batch 2, Batch 3)
...
- The `--platform` argument adds platform-specific Codex user-input prompt batches and a
