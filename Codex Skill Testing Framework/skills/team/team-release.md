...
- [ ] Uses `Codex user-input prompt` at phase transitions requiring user approval before proceeding
...
**Expected behavior:**
1. Phase 1: Spawns `producer` via a Codex subagent; confirms all milestone acceptance criteria met; identifies any deferred scope; produces release authorization; presents to user; Codex user-input prompt: user approves before Phase 2
2. Phase 2: Spawns `release-manager` via a Codex subagent; cuts release branch from agreed commit; bumps version numbers; invokes `/release-checklist`; freezes branch; output: branch name and checklist; Codex user-input prompt: user approves before Phase 3
3. Phase 3 (parallel): Issues Codex subagent calls simultaneously for `qa-lead` (regression suite, critical path sign-off) and `devops-engineer` (build artifacts, CI verification); security-engineer is NOT spawned (no online features); network-programmer is NOT spawned (no multiplayer); both complete successfully
...
5. Phase 5: Spawns `producer` via a Codex subagent; collects sign-offs from qa-lead, release-manager, devops-engineer; no open blocking issues; producer declares GO; Codex user-input prompt: user sees GO decision and confirms deployment
...
**Assertions:**
- [ ] Phase 3 qa-lead and devops-engineer Codex subagent calls are issued simultaneously, not sequentially
...
4. Phase 4 proceeds cautiously or is paused (Codex user-input prompt: continue to Phase 4 or skip to Phase 5 for go/no-go?)
5. Phase 5: Spawns `producer` via a Codex subagent; producer receives qa-lead's NOT PASSED verdict; no S1 sign-off available; producer declares NO-GO with rationale: "S1 bug [ID] is open and unresolved. Releasing is not safe."
6. Codex user-input prompt: user is presented with the NO-GO decision and the S1 bug details; options: fix the bug and re-run, defer the release, or override (with documented rationale)
...
- [ ] Codex user-input prompt offers the user resolution options (fix and re-run / defer / override with rationale)
...
2. Phase 3 (parallel): Orchestrator detects that the game has online/multiplayer features and player data; issues Codex subagent calls simultaneously for `qa-lead`, `devops-engineer`, AND `security-engineer`; also spawns `network-programmer` for netcode stability sign-off
...
- [ ] All four Phase 3 Codex subagent calls (qa-lead, devops-engineer, security-engineer, network-programmer) are issued simultaneously
...
4. Codex user-input prompt: options presented — (a) Fix translations and re-run Phase 4, (b) Remove French locale from this release, (c) Ship as-is with a known issues note
...
- [ ] Codex user-input prompt is used to offer the user resolution choices — the skill does not auto-waive
...
3. Confirms with Codex user-input prompt before beginning Phase 1 proper: "Releasing v1.1.0. Is this correct?"
...
2. Uses Codex user-input prompt: "What version number should be released? (e.g., v1.0.0)"
...
- [ ] Inferred version is confirmed with the user via Codex user-input prompt before proceeding (variant A)
- [ ] When no version is discoverable, Codex user-input prompt is used — skill does not guess (variant B)
...
- [ ] `Codex user-input prompt` used at each phase transition gate (post-Phase 1, post-Phase 2, post-Phase 3/4 if issues, post-Phase 5 go/no-go)
- [ ] Phase 3 agents are always issued as parallel Codex subagent calls — qa-lead and devops-engineer are never sequential
...
- The "devops-engineer build fails" path is not separately tested — it would surface as a BLOCKED result in Phase 3 and follow the standard error recovery protocol (surface → assess → Codex user-input prompt options). This is validated structurally by the Static Assertions error recovery check.
...
- The "override NO-GO with documented rationale" path in Case 2 is referenced but not exhaustively tested — it is an escape hatch that the skill must support, and its existence is validated by the Codex user-input prompt options assertion in Case 2.
