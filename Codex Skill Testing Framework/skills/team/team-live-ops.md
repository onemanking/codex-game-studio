...
**Expected behavior:**
1. Phase 1: Spawns `live-ops-designer` via a Codex subagent; receives season brief with scope, content list, and retention mechanic; presents to user
2. Codex user-input prompt: user approves Phase 1 output before Phase 2 begins
3. Phase 2: Spawns `narrative-director` via a Codex subagent; reads the Phase 1 season brief; produces narrative framing document (theme, story hook, lore connections); presents to user
4. Phase 3 and 4 (parallel): Spawns `economy-designer` and `analytics-engineer` simultaneously via two Codex subagent calls before waiting for either result; economy-designer reads `design/live-ops/economy-rules.md`
...
6. Phase 6: Spawns `community-manager` via a Codex subagent; reads season brief, economy design, and narrative framing; produces communication calendar with draft copy
...
8. Codex user-input prompt: user approves the full season plan
...
**Assertions:**
- [ ] All 7 phases execute in order; Phase 3 and 4 are issued as parallel Codex subagent calls
...
4. Codex user-input prompt presented with resolution options before season plan approval is offered
...
- [ ] Codex user-input prompt is used to surface the violation and offer resolution options (revise economy design, override with documented rationale, cancel)
...
- [ ] No Codex subagent calls are issued before the argument check fails
...
**Expected behavior:**
1. After Phase 2 is approved by the user, the orchestrator issues both Codex subagent calls (economy-designer and analytics-engineer) before awaiting either result
...
**Assertions:**
- [ ] Both Codex subagent calls for Phase 3 and Phase 4 are issued before either result is awaited — they are not sequential
- [ ] Analytics-engineer prompt does NOT include economy-designer output as a required input (the inputs are independent)
- [ ] If economy-designer blocks but analytics-engineer succeeds, analytics output is preserved and the block is surfaced via Codex user-input prompt
...
- [ ] `Codex user-input prompt` used at every phase transition — user approves before the next phase begins
...
- [ ] Error recovery: any BLOCKED agent is surfaced immediately with Codex user-input prompt options (skip / retry / stop)
