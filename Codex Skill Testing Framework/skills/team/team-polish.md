...
and issues READY FOR RELEASE or NEEDS MORE WORK). Uses `Codex user-input prompt` at each
...
2. `Codex user-input prompt` presents performance report; user approves before Phases 2, 3, and 4 begin
...
5. All three parallel phases complete; `Codex user-input prompt` presents results; user approves before Phase 5 begins
6. Phase 5: qa-tester runs edge case tests, soak tests, stress tests, and regression tests; all pass
7. `Codex user-input prompt` presents test results; user approves before Phase 6
...
- [ ] `Codex user-input prompt` appears after Phase 1 output and before Phases 2/3/4 launch
- [ ] Phases 3 and 4 Codex subagent calls are issued at the same time as Phase 2 (not after Phase 2 completes)
...
2. `Codex user-input prompt` presents the violation; user chooses to proceed with optimization attempt
...
3. `Codex user-input prompt` presents the performance report including the engine-level root cause; user approves before Phase 2
...
- [ ] engine-programmer and performance-analyst Codex subagent calls in Phase 2 are issued simultaneously (not sequentially)
...
7. `Codex user-input prompt` presents the regression with options:
...
- [ ] `Codex user-input prompt` offers options including revert, fix in place, and schedule later
