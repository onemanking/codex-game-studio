...
`Codex user-input prompt` at each step transition. Verdict is COMPLETE when the audio
...
- [ ] `Codex user-input prompt` is used at step transitions before proceeding
...
3. `Codex user-input prompt` presents audio direction; user approves before Step 2 begins
...
5. `Codex user-input prompt` presents SFX spec and accessibility requirements; user approves before Step 3 begins
...
7. `Codex user-input prompt` presents technical plan; user approves before Step 4 begins
...
- [ ] `Codex user-input prompt` appears after Step 1 output and before Step 2 launch
- [ ] sound-designer and accessibility-specialist Codex subagent calls are issued simultaneously in Step 2
- [ ] technical-artist and engine specialist Codex subagent calls are issued simultaneously in Step 3
- [ ] gameplay-programmer is not launched until Step 3 `Codex user-input prompt` is approved
...
3. Orchestrator surfaces the concern immediately in conversation before presenting `Codex user-input prompt`
4. `Codex user-input prompt` presents the accessibility concern as a BLOCKING issue with options:
...
- [ ] `Codex user-input prompt` surfaces the gap before Step 3 is launched
...
- [ ] `Codex user-input prompt` is used after every step output before the next step launches
- [ ] Parallel spawning: Step 2 (sound-designer + accessibility-specialist) and Step 3 (technical-artist + engine specialist) issue all Codex subagent calls before waiting for results
