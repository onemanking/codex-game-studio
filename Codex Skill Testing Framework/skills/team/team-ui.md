...
Uses `Codex user-input prompt` at each phase transition. Delegates all file writes to
...
2. Phase 1b — `/ux-design inventory-screen` invoked (or ux-designer spawned directly); produces `design/ux/inventory-screen.md` using `ux-spec.md` template; `Codex user-input prompt` confirms spec before review
3. Phase 1c — `/ux-review design/ux/inventory-screen.md` invoked; returns APPROVED; gate passed, proceed to Phase 2
4. Phase 2 — art-director spawned; reviews full UX spec (not only wireframes); applies visual treatment; verifies color contrast; produces visual design spec with asset manifest; `Codex user-input prompt` confirms before Phase 3
...
4. `Codex user-input prompt` presented with the specific flagged concerns and options:
...
- [ ] `Codex user-input prompt` presents the specific flagged concerns before offering options
...
2. Three Codex subagent calls issued simultaneously: ux-designer, art-director, accessibility-specialist
...
5. `Codex user-input prompt` presents all three review results before Phase 5 begins

**Assertions:**
- [ ] All three Codex subagent calls issued before any result is awaited (parallel, not sequential)
...
- [ ] `Codex user-input prompt` shows all three review streams' results together before Phase 5 approval
...
3. `Codex user-input prompt` presented with options:
...
- [ ] `Codex user-input prompt` offers a "create pattern library first" option (referencing `/ux-design patterns`)
