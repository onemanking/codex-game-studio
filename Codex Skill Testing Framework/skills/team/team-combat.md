...
(parallel) → Integration → Validation → Sign-off. Uses `Codex user-input prompt` at each
...
- [ ] Uses `Codex user-input prompt` at phase transitions for user approval before proceeding
...
2. Phase 2 — gameplay-programmer + ai-programmer spawned; produce architecture sketch with class structure, interfaces, and file list; then primary engine specialist is spawned to validate idioms; engine specialist output incorporated; `Codex user-input prompt` presented with architecture options before Phase 3 begins
...
4. Phase 4 — integration wires together all Phase 3 outputs; tuning knobs verified as data-driven; `Codex user-input prompt` confirms integration before Phase 5
...
**Assertions:**
- [ ] `Codex user-input prompt` called at each phase gate (at minimum before Phase 3 and before Phase 5)
...
4. `Codex user-input prompt` presented with options: (a) Skip ai-programmer and note the gap; (b) Retry with narrower scope; (c) Stop here and run `/architecture-decision` first
...
2. All four Codex subagent calls — gameplay-programmer, ai-programmer, technical-artist, sound-designer — are issued before any result is awaited
...
**Assertions:**
- [ ] Four Codex subagent calls issued in a single batch (no sequential waiting between them)
...
6. `Codex user-input prompt` includes engine specialist's notes alongside the architecture sketch
...
- [ ] `Codex user-input prompt` used at each phase transition — user approves before pipeline advances
