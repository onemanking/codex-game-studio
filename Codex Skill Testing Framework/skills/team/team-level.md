...
document saved to `design/levels/[level-name].md`. Uses `Codex user-input prompt` at each
...
- [ ] Uses `Codex user-input prompt` at step transitions for user approval before proceeding
...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `Codex user-input prompt` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `Codex user-input prompt` confirms layout before Step 3
4. Step 3 — systems-designer spawned: specifies enemy compositions, loot tables, difficulty balance, area-specific mechanics, resource distribution; `Codex user-input prompt` confirms systems before Step 4
5. Step 4 — art-director and accessibility-specialist spawned in parallel; art-director: visual theme, color palette, lighting, asset list, VFX needs; accessibility-specialist: navigation clarity, colorblind safety, cognitive load check — each concern rated BLOCKING / RECOMMENDED / NICE TO HAVE; `Codex user-input prompt` presents both outputs before Step 5
...
- [ ] `Codex user-input prompt` called at each step gate (minimum: after Step 1, Step 2, Step 3, Step 4)
...
4. `Codex user-input prompt` presented with options:
...
- [ ] `Codex user-input prompt` offers at minimum three options (skip / retry / stop)
...
4. Skill presents both Step 4 results via `Codex user-input prompt` — BLOCKING concern highlighted prominently
5. `Codex user-input prompt` offers:
...
- [ ] `Codex user-input prompt` presents the specific concern text (not just "accessibility issue found")
...
- [ ] `Codex user-input prompt` offers a "design crystal caves first" option referencing `/team-level`
...
- [ ] `Codex user-input prompt` used at each step transition — user approves before pipeline advances
- [ ] All file writes delegated to sub-agents via a Codex subagent — orchestrator does not call Write or Edit directly
