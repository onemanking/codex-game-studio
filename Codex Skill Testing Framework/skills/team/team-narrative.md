...
and world-builder in parallel). Uses `Codex user-input prompt` at each phase transition to
...
- [ ] `Codex user-input prompt` is used at phase transitions before proceeding
...
2. `Codex user-input prompt` presents the narrative brief; user approves before Phase 2 begins
...
4. `Codex user-input prompt` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `Codex user-input prompt` presents level narrative plan; user approves before Phase 4 begins
...
8. `Codex user-input prompt` presents review results; user approves before Phase 5 begins
...
- [ ] `Codex user-input prompt` appears after Phase 1 output and before Phase 2 launch
- [ ] world-builder and writer Codex subagent calls are issued simultaneously in Phase 2 (not sequentially)
- [ ] level-designer is not launched until Phase 2 `Codex user-input prompt` is approved
...
6. `Codex user-input prompt` presents options:
...
- [ ] `Codex user-input prompt` presents at least 3 options including "stop and resolve first"
...
- [ ] No `Codex user-input prompt` is used — output is direct guidance
...
5. `Codex user-input prompt` presents options:
...
- [ ] `Codex user-input prompt` offers the option to fix now vs. flag and proceed
...
- [ ] `Codex user-input prompt` offers at least one option to resolve the missing prerequisite
...
- [ ] `Codex user-input prompt` is used after every phase output before the next phase launches
- [ ] Parallel spawning: Phase 2 (world-builder + writer) and Phase 5 (writer + localization-lead + world-builder) issue all Codex subagent calls before waiting for results
...
- The "Retry with narrower scope" and "Skip this agent" resolution paths from the Error
  Recovery Protocol are not separately tested — they follow the same `Codex user-input prompt`
