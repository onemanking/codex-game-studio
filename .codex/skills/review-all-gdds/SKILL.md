...
Use `Codex user-input prompt` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `Codex user-input prompt`:
...
After all file writes are complete, use `Codex user-input prompt` for a closing widget.
