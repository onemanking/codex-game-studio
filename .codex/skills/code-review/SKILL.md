...
name: code-review
description: >-
  Codex Game Studio skill 'code-review'. Performs an architectural and quality code review on a specified file or set of files. Checks for coding standard compliance, architectural pattern adherence, SOLID principles, testability, and performance concerns.
---
...
Spawn all applicable specialists simultaneously via a Codex subagent — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` via a Codex subagent in parallel with the engine specialists. Pass:
