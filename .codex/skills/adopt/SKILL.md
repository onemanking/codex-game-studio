...
name: adopt
description: >-
  Codex Game Studio skill 'adopt'. Brownfield onboarding — audits existing project artifacts for template format compliance (not just existence), classifies gaps by impact, and produces a numbered migration plan. Run this when joining an in-progress project or upgrading from an older template version. Distinct from /project-stage-detect (which checks what exists) — this checks whether what exists will actually work with the template's skills.
---
...
If the project appears fresh (no artifacts at all), use `Codex user-input prompt`:
...
and offer to handle it immediately using `Codex user-input prompt`. Choose the first
branch that applies:
