...
name: map-systems
description: >-
  Codex Game Studio skill 'map-systems'. Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index.
---
...
- Use `Codex user-input prompt` to ask:
...
Then use `Codex user-input prompt` to capture feedback:
...
Use `Codex user-input prompt` to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via a Codex subagent using gate TD-SYSTEM-BOUNDARY (`.codex/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use `Codex user-input prompt` to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via a Codex subagent using gate PR-SCOPE (`.codex/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via a Codex subagent using gate CD-SYSTEMS (`.codex/docs/director-gates.md`).**
...
Use `Codex user-input prompt` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `Codex user-input prompt`:
...
After the systems index is created (or after designing some systems), present next actions using `Codex user-input prompt`:
