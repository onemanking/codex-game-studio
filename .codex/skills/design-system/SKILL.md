...
name: design-system
description: >-
  Codex Game Studio skill 'design-system'. Guided, section-by-section GDD authoring for a single game system. Gathers context from existing docs, walks through each required section collaboratively, cross-references dependencies, and writes incrementally to file.
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `Codex user-input prompt`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `Codex user-input prompt` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `Codex user-input prompt` to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `Codex user-input prompt`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `Codex user-input prompt` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `Codex user-input prompt`:
...
spawn `creative-director` via a Codex subagent:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via a Codex subagent in parallel:
...
- Surface any disagreements between agents to the user via `Codex user-input prompt`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via a Codex subagent in parallel:
...
- Present the specialists' proposals to the user for review via `Codex user-input prompt`
...
**Agent delegation (MANDATORY)**: Spawn `qa-lead` via a Codex subagent before finalising acceptance criteria. Provide: the completed GDD sections C, D, E, and ask them to validate that the criteria are independently testable and cover all core rules and formulas. Surface any gaps or untestable criteria to the user.
...
For required systems: **spawn `art-director` via a Codex subagent** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via a Codex subagent using gate **CD-GDD-ALIGN** (`.codex/docs/director-gates.md`).
...
**When delegating via Codex subagent tool**:
...
- The main session presents the agent's output to the user via `Codex user-input prompt`
- The user decides; the main session writes to file
...
2. **Codex user-input prompt** at every decision point (Explain -> Capture pattern):
