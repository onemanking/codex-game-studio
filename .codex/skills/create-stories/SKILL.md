...
name: create-stories
description: >-
  Codex Game Studio skill 'create-stories'. Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /create-epics for each epic.
---
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via a Codex subagent using gate **QL-STORY-READY** (`.codex/docs/director-gates.md`).
...
Use `Codex user-input prompt` to close with context-aware next steps:
