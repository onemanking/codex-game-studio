...
If they differ, use `Codex user-input prompt` before proceeding:
...
specialist to spawn via a Codex subagent.
...
Spawn the chosen programmer agent(s) via a Codex subagent with the full context package:
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via a Codex subagent. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
