...
name: brainstorm
description: >-
  Codex Game Studio skill 'brainstorm'. Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration.
---
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `Codex user-input prompt`** at key decision points throughout brainstorming:
   ...
   `Codex user-input prompt` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an Codex user-input prompt with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `Codex user-input prompt` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `Codex user-input prompt` with these exact tab labels:
...
Present all three. Then use `Codex user-input prompt` to capture the selection.
...
Ask these as `Codex user-input prompt` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `Codex user-input prompt`:
...
If the user selects B, C, or D, make the revision, then use `Codex user-input prompt` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via a Codex subagent in parallel before moving to Phase 5. Issue both Codex subagent calls simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-tab `Codex user-input prompt`:
...
- **Target platform**: Use `Codex user-input prompt` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `Codex user-input prompt` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via a Codex subagent using gate TD-FEASIBILITY (`.codex/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via a Codex subagent using gate PR-SCOPE (`.codex/docs/director-gates.md`).**
...
5. Use `Codex user-input prompt` for write approval:
...
If [B]: ask which section to revise using `Codex user-input prompt` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `Codex user-input prompt` — "Ready to write the updated concept document?"
