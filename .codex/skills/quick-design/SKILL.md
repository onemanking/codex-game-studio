...
name: quick-design
description: >-
  Codex Game Studio skill 'quick-design'. Lightweight design spec for small changes — tuning adjustments, minor mechanics, balance tweaks. Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files.
---
...
Present the inferred classification using `Codex user-input prompt`:
...
Present the draft to the user in full. Then use `Codex user-input prompt`:
