...
name: design-review
description: >-
  Codex Game Studio skill 'design-review'. Reviews a game design document for completeness, internal consistency, implementability, and adherence to project design standards. Run this before handing a design document to programmers.
---
...
Use `Codex user-input prompt` for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab `Codex user-input prompt` before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use `Codex user-input prompt` for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single `Codex user-input prompt` with `multiSelect: true` to batch the two tracking updates:
...
Use a second `Codex user-input prompt`:
...
Use a third `Codex user-input prompt`:
...
Once the systems-index and review-log widgets are answered, check project state and show one final `Codex user-input prompt`:
