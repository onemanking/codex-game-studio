...
name: hotfix
description: >-
  Codex Game Studio skill 'hotfix'. Emergency fix workflow that bypasses normal sprint processes with a full audit trail. Creates hotfix branch, tracks approvals, and ensures the fix is backported correctly.
---
...
Confirm with `Codex user-input prompt`:
...
If the check passes, use `Codex user-input prompt` before creating the branch:
...
Use the Codex subagent tool to request sign-off in parallel:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via a Codex subagent with:
