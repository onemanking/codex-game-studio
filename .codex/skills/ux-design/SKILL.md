...
name: ux-design
description: >-
  Codex Game Studio skill 'ux-design'. Guided, section-by-section UX spec authoring for a screen, flow, or HUD. Reads game concept, player journey, and relevant GDDs to provide context-aware design guidance. Produces ux-spec.md (per screen/flow) or hud-design.md using the studio templates.
---
...
**If no argument is provided**, do not fail — ask instead. Use `Codex user-input prompt`:
...
2. **Questions**: Ask what is needed to draft this section. Use `Codex user-input prompt`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `Codex user-input prompt` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `Codex user-input prompt`:
...
7. **Write**: Use `Codex user-input prompt`: "May I write the [section name] section to `[filepath]`?"
...
- Use `Codex user-input prompt` to capture the choice:
...
- Use `Codex user-input prompt`: "Want an ASCII wireframe as part of this spec?"
...
Use `Codex user-input prompt` to confirm:
...
Use `Codex user-input prompt` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `Codex user-input prompt`:
...
Then use `Codex user-input prompt`:
...
When delegating to another agent via the Codex subagent tool:
