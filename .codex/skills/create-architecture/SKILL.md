...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `Codex user-input prompt`:
...
**Step 2 — Spawn `lead-programmer` via a Codex subagent using gate LP-FEASIBILITY (`.codex/docs/director-gates.md`):**
...
Use `Codex user-input prompt` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `Codex user-input prompt`:
