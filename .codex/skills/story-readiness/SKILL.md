...
description: >-
  Codex Game Studio skill 'story-readiness'. Validate that a story file is implementation-ready. Checks for embedded GDD requirements, ADR references, engine notes, clear acceptance criteria, and no open design questions. Produces READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I start on this story', 'is story X ready to implement'.
---
...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via Codex user-input prompt)
...
If no argument is given, use `Codex user-input prompt`:
...
Spawn `qa-lead` via a Codex subagent using gate **QL-STORY-READY** (`.codex/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via `Codex user-input prompt`:
