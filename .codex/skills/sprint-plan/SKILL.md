...
name: sprint-plan
description: >-
  Codex Game Studio skill 'sprint-plan'. Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs.
---
...
- If the file doesn't exist and this is a `new` sprint: use `Codex user-input prompt`:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use `Codex user-input prompt` to gather changes.
...
Before finalising the sprint plan, spawn `producer` via a Codex subagent using gate **PR-SPRINT** (`.codex/docs/director-gates.md`).
...
If CONCERNS, use `Codex user-input prompt`:
