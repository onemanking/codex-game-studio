...
name: start
description: >-
  Codex Game Studio skill 'start'. First-time onboarding — asks where you are, then guides you to the right workflow. No assumptions.
---
...
This is the first thing the user sees. Use `Codex user-input prompt` with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not Codex user-input prompt (it's an open response).
2. Acknowledge the concept, then use `Codex user-input prompt` to offer two paths:
...
**If it does not exist**: Use `Codex user-input prompt`:
...
After presenting the recommended path, use `Codex user-input prompt` to ask the user which step they'd like to take first. Never auto-run the next skill.
