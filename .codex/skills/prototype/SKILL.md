...
name: prototype
description: >-
  Codex Game Studio skill 'prototype'. Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after /brainstorm and /setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict.
---
...
Otherwise, use `Codex user-input prompt` to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then use `Codex user-input prompt` with your
...
- `full` → spawn `creative-director` via a Codex subagent using gate **CD-PLAYTEST** if
...
2. **Choose path** — same Codex user-input prompt widget as Phase 3 (HTML / Engine / Paper).
