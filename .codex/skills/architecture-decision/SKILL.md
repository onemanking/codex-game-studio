...
description: >-
  Codex Game Studio skill 'architecture-decision'. Creates an Architecture Decision Record (ADR) documenting a significant technical decision, its context, alternatives considered, and consequences. Every major technical choice should have an ADR.
---
...
a **confirm/adjust** prompt using `Codex user-input prompt` — not open-ended questions.
...
decisions remain, present each one as a separate `Codex user-input prompt` with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via a Codex subagent to validate the drafted ADR:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via a Codex subagent using gate **TD-ADR** (`.codex/docs/director-gates.md`):
...
5. **Write approval** — Use `Codex user-input prompt`:
...
Ask using `Codex user-input prompt`:
...
After the ADR is written (and registry optionally updated), close with `Codex user-input prompt`.
