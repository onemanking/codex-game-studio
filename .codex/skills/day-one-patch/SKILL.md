...
Use `Codex user-input prompt` to confirm proceeding or reduce scope.
...
Spawn `release-manager` via a Codex subagent. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` via a Codex subagent with:
...
3. Spawn `qa-tester` via a Codex subagent to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` via a Codex subagent with:
