...
description: >-
  Codex Game Studio skill 'architecture-review'. Validates completeness and consistency of the project architecture against all GDDs. Builds a traceability matrix mapping every GDD technical requirement to ADRs, identifies coverage gaps, detects cross-ADR conflicts, verifies engine compatibility consistency across all decisions, and produces a PASS/CONCERNS/FAIL verdict. The architecture equivalent of /design-review.
---
...
After completing the engine audit above, spawn the **primary engine specialist** via a Codex subagent for a domain-expert second opinion:
...
If [B]: display the complete proposed systems-index section, then re-ask with `Codex user-input prompt`.
...
Use `Codex user-input prompt` for the write approval:
...
For `rtm` mode, use `Codex user-input prompt`:
...
Then close with `Codex user-input prompt` tailored to the pre-gate checklist state:
...
3. **Offer options** via Codex user-input prompt with three choices:
...
5. **Use `Codex user-input prompt` for write approvals** — plain text "May I?" is not
