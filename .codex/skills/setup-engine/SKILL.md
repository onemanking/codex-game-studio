...
name: setup-engine
description: >-
  Codex Game Studio skill 'setup-engine'. Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data.
---

# Codex Game Studio Skill

> Adapted for workspace-local Codex use.
...
- **Web Browser family**: `/setup-engine web` or `/setup-engine web hybrid` — browser engine family and optional stack profile provided
...
**Question 1 — Prior experience** (ask this first, always, via `Codex user-input prompt`):
- Prompt: "Have you worked in any of these engines before?"
- Options: `Godot` / `Unity` / `Unreal Engine 5` / `Web Browser` / `Multiple — I'll explain` / `None of them`
...
**Question 2 — Target platform** (ask this second, always, via `Codex user-input prompt` — platform eliminates or heavily weights engines before any other factor):
...
- Platform rules that feed directly into the recommendation:
  ...
  - Web → Evaluate Web Browser first. Phaser, Three.js, React Three Fiber, or a hybrid browser stack is the native fit. Godot/Unity web exports are viable only when the project intentionally starts from those engines. Unreal is a poor browser fit.
...
**Web Browser**
- Genuine strengths: No install, shareable URL, fast iteration, strong 2D and 3D library ecosystem, DOM/canvas/WebGL composition, easy playable prototypes, and natural deployment for training sims, portfolio games, and web-first games
- Real limitations: Browser/device variability, mobile viewport constraints, audio unlock requirements, asset budget pressure, WebGL context loss, performance ceilings, weaker built-in editor workflows, and more architecture responsibility on the project
- Licensing reality: Browser stack costs depend on libraries and hosting. Phaser, Three.js, React Three Fiber, and Vite-style tooling are commonly open-source, but hosted services and asset licenses still need review.
- Best fit: Browser-first 2D/3D/hybrid games, playable prototypes, web-first training sims, shareable demos, React-hosted game UIs, Phaser arcade/tilemap games, Three.js/R3F 3D experiences, and mixed 2D/3D browser surfaces
- Stack profiles: Phaser 2D, plain Three.js 3D, React Three Fiber, or Hybrid Phaser + Three/R3F + DOM UI
...
- Browser-first / shareable URL / playable embed → Web Browser strongly preferred
- Hybrid 2D UI plus 3D scene in a web app → Web Browser hybrid profile
...
5. Use `Codex user-input prompt` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `Codex user-input prompt` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
- If the chosen engine family is **Web Browser**, do not look up a single engine
  version. Inspect `package.json` if it exists, or ask which stack profile to
  start with. Pin relevant package versions such as `phaser`, `three`,
  `@react-three/fiber`, `vite`, physics, state, and asset-pipeline packages
  only when they are actually added.
- For Godot/Unity/Unreal, if version was provided, use it
- For Godot/Unity/Unreal, if no version provided, use WebSearch to find the latest stable release:
...
- **Web Stack Profile**: [Phaser 2D / Three.js 3D / React Three Fiber / Hybrid]
- **Language**: TypeScript
- **Build System**: Vite or existing project build system
- **Runtime Surface**: Canvas/WebGL plus DOM overlay as needed
- **Asset Pipeline**: Web assets, GLB/glTF, texture compression, sprite atlases, audio optimized for browser delivery
```
...
**For Web Browser (TypeScript/JavaScript):**
- Components/classes: PascalCase (e.g., `CombatScene`, `PlayerController`)
- Functions/variables: camelCase (e.g., `spawnEnemy`, `moveSpeed`)
- Constants: UPPER_SNAKE_CASE for global constants, camelCase for local readonly values
- Files: kebab-case for assets and framework-agnostic modules, PascalCase for React components where the existing project uses that convention
- Events/actions: namespaced strings or typed action objects (e.g., `player:damaged`, `PLAYER_JUMP`)
...
### Remaining Sections
- **Performance Budgets**: Use `Codex user-input prompt`:
...
- **Testing**: Suggest engine-appropriate framework (GUT for Godot, NUnit for Unity, Unreal automation tests, or browser build/playtest checks with Playwright/Vitest as appropriate) — ask before adding.
...
- **Routing Notes**: Treat Web Browser as an engine family. Invoke web-game-specialist for stack decisions, hybrid runtime boundaries, loop ownership, input/action bus, asset lifecycle, audio unlock, storage, viewport constraints, and browser deployment. Invoke browser-qa-specialist before claiming rendered browser behavior is verified.

### File Extension Routing

| File Extension / Type | Specialist to Spawn |
|-----------------------|---------------------|
| Browser runtime files (.ts, .tsx, .js, .jsx) | web-game-specialist, then stack specialist |
| Phaser scenes/systems | phaser-specialist |
| Three.js scene/runtime files | threejs-specialist |
...
- Web Browser: do not treat the browser family as one version. Pin and verify the package stack from `package.json` or official package docs (`phaser`, `three`, `@react-three/fiber`, `vite`, physics, state, and asset-pipeline packages).
