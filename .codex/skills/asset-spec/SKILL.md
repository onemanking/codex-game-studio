...
name: asset-spec
description: >-
  Codex Game Studio skill 'asset-spec'. Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after art bible and GDD/level design are approved, before production begins.
---
...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `Codex user-input prompt`:
...
Present the full proposed inventory to the user in conversation. Then use `Codex user-input prompt`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `Codex user-input prompt`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `Codex user-input prompt`:
...
Present the full identified list to the user. Use `Codex user-input prompt`:
...
**`art-director`** via a Codex subagent:
...
**`technical-artist`** via a Codex subagent:
...
After presenting all specs, use `Codex user-input prompt`:
