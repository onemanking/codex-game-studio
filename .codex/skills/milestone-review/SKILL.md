...
description: >-
  Codex Game Studio skill 'milestone-review'. Generates a comprehensive milestone progress review including feature completeness, quality metrics, risk assessment, and go/no-go recommendation. Use at milestone checkpoints or when evaluating readiness for a milestone deadline.
---
...
Before generating the Go/No-Go recommendation, spawn `producer` via a Codex subagent using gate **PR-MILESTONE** (`.codex/docs/director-gates.md`).
...
If OFF TRACK, use `Codex user-input prompt` before generating the recommendation:
...
If AT RISK, use `Codex user-input prompt`:
