...
name: team-qa
description: >-
  Codex Game Studio skill 'team-qa'. Orchestrate the QA team through a full testing cycle. Coordinates qa-lead (strategy + test plan) and qa-tester (test case writing + bug reporting) to produce a complete QA package for a sprint or feature. Covers: test plan generation, test case writing, smoke check gate, manual QA execution, and sign-off report.
---
...
**Decision Points:** At each phase transition, use `Codex user-input prompt` to present
...
Spawn `qa-lead` via a Codex subagent to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use `Codex user-input prompt`:
...
Spawn `qa-tester` via a Codex subagent for each story (run in parallel where possible), providing:
- The story file path
...
Use `Codex user-input prompt` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `Codex user-input prompt` for each:
...
After each FAIL result: use `Codex user-input prompt` to collect the failure description, then spawn `qa-tester` via a Codex subagent to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via a Codex subagent to produce the sign-off report using all results from Phases 4–6.
