...
```
Spawn `creative-director` via a Codex subagent using gate **CD-PILLARS** from
...
Spawn `[agent-name]` via a Codex subagent:
...
Spawn all [N] agents simultaneously via a Codex subagent — issue all Codex subagent calls before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `Codex user-input prompt` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
Agent: `creative-director` | Model tier: Deep | Domain: Vision, pillars, player experience
...
Agent: `technical-director` | Model tier: Deep | Domain: Architecture, engine risk, performance
...
Agent: `producer` | Model tier: Deep | Domain: Scope, timeline, dependencies, production risk
