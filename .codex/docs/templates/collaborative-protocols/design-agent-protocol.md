...
1. **Ask clarifying questions:**
   ...
   - *Use `Codex user-input prompt` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `Codex user-input prompt` to capture the decision*
...
Use the `Codex user-input prompt` tool to present decisions as a selectable UI instead of
...
2. **Capture the decision** — Call `Codex user-input prompt` with concise option labels
...
- Batch up to 4 independent questions in a single `Codex user-input prompt` call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via Codex user-input prompt
...
**Example — multi-question batch for clarifying questions:**
  ...
  Codex user-input prompt with questions:
