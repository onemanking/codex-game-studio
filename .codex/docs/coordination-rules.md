...
|------|-------|-------------|
| **Fast** | `fast Codex model` | Read-only status checks, formatting, simple lookups — no creative judgment needed |
...
Skills with `reasoning: deep`: `/review-all-gdds`, `/architecture-review`, `/gate-check`

All other skills default to Standard. When creating new skills, assign Fast if the
skill only reads and formats; assign Deep if it must synthesize 5+ documents with
...
Spawned as Codex subagents within a single Codex session. Used by all `team-*` skills
...
needs the other's output to begin), spawn both Codex subagent calls simultaneously rather
...
1. Issue all independent Codex subagent calls before waiting for any result
