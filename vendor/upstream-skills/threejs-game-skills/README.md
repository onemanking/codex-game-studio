...
If your installed `skills` CLI does not support the Codex target, install from a cloned checkout with `./install.sh --codex`.
...
After installing, open Codex or Codex in an empty project folder, or in an existing Three.js game you want to improve. Then prompt the agent with the outcome you want and name the director skill:
...
After setting persistent Windows variables, restart your terminal, Codex, or Codex so the agent process can see the new environment.
