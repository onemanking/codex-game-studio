# Three.js Skills for Codex

A curated collection of Three.js skill files that provide Codex with foundational knowledge for creating 3D elements and interactive experiences.

## Purpose

When working with Three.js, Codex starts with general programming knowledge but lacks specific Three.js API details, best practices, and common patterns. These skill files bridge that gap by providing:
...
Clone this repository into your project or copy the `.codex/skills` directory:
...
Codex automatically loads skill files from the `.codex/skills` directory when they match the context of your request. When you ask Codex to:
...
Codex will use `threejs-fundamentals` to generate accurate boilerplate with proper renderer setup, animation loop, and resize handling.
...
Codex will use `threejs-loaders` and `threejs-animation` to generate code with proper loader configuration, animation mixer setup, and error handling.
...
Codex will use `threejs-shaders` to generate working GLSL code with proper uniform declarations and coordinate space handling.
...
2. Edit or create skill files in `.codex/skills/`
